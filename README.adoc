...
correctly with touch-driven interfaces, and other such capabilities. Some of
these may be impossible or at the very least, extremely hard, to solve with this
approach.
...
==== Native CRDT Characters

The internal primary data model for the editor is the causal tree CRDT described
above. See the
https://github.com/ritzyed/ritzy/blob/master/docs/DESIGN.adoc[DESIGN] for more
details.

This data is accessible via the
...
and https://github.com/ritzyed/ritzy/blob/master/docs/API.adoc#selections[API
Selection].

===== Rich Text JSON

Ritzy can also import and export a rich text JSON model. The JSON structure
contains an array of text chunks with associated attributes. These attributes
are currently boolean values for bold, italics, and so forth, but could in the
future include other values such as style names.
...
See `client.js` or the https://github.com/ritzyed/ritzy-demo[Ritzy demo] for an
example of creating and using the editor. Note that `client.js` uses the `ritzy`
module, which wraps the underlying React `Editor` component. It provides a
useful https://github.com/ritzyed/ritzy/blob/master/docs/API.adoc[API] to
calling code, including methods to control the editor at runtime and event
registration.

NOTE: If you are building a React app, you are free to embed the `Editor`
component directly rather than using the `ritzy` module.
